    ...
    <div id="slide-window" v-if="showDetialBoxCon">
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="基本情况" name="基本情况">
          <el-form v-loading="loading" element-loading-text="拼命加载中..." :model="customerDetail" :rules="rules" ref="customerDetail"
          ...
          <div class="historyBox">
            ...
            <div v-if="followupLog.length>0" style="max-height: 300px;overflow-y: auto">
              <div v-for="item in followupLog">
                <p>
                  <span>{{item.followupTime/1000 | moment("YYYY-MM-DD HH:mm:ss")}}</span>
                  <span>{{item.classId}}</span>
                  <span>{{callResult[item.followupId]}}</span>
                </p>
        ...
        <el-tab-pane label="功能使用" name="功能使用">
          <div v-loading="loading" element-loading-text="拼命加载中...">
            <div class="mealAllowancebox">
              ...
              <div>
                <p>
                  <span>2018-01-17 13:48:30</span>
                  <span>重庆市(113.251.19.68)</span>
                  <span>生成平面图</span>
                </p>
                <p>
                  <span>2018-01-17 13:48:30</span>
              ...
              <div class="block" v-show="customerFunctionLog1.length>0" style="text-align: center;margin-top:20px">
                <el-pagination @size-change="handleSizeChange1" @current-change="handleCurrentChange1" :current-page="customerFunctionLog1.page"
                  :page-sizes="[10, 20, 50, 100]" :page-size="customerFunctionLog1.size" layout="total, sizes, prev, pager, next, jumper"
            ...
            <div class="mealAllowancebox">
              ...
              <div class="block" v-show="customerFunctionLog2.length>0" style="text-align: center;margin-top:20px">
                <el-pagination @size-change="handleSizeChange2" @current-change="handleCurrentChange2" :current-page="customerFunctionLog2.page"
                  :page-sizes="[10, 20, 50, 100]" :page-size="customerFunctionLog2.size" layout="total, sizes, prev, pager, next, jumper"
              ...
              <div class="mealAllowancebox">
                ...
                <div>
                  ...
                  <p class="floatRightBox" v-for="item in topOrderList">
                    订单编号：
                    <span style="cursor: pointer;color:#409EFF" @click="searchNumBtn(item.outTradeOrderId)">{{item.outTradeOrderId}}</span>
                    <el-button type="primary floatRight" @click="delBindOrder(item.outTradeOrderId)">解除绑定</el-button>
                  </p>
                  </p>
                </div>
              </div>
        ...
        <el-tab-pane label="挖掘记录" name="挖掘记录">
          <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="tableData" border style="width: 100%">
            <el-table-column prop="dmtime" label="挖掘时间" width="180">
              <template slot-scope="scope">{{ scope.row.dmtime/1000 |moment("YYYY-MM-DD HH:mm:ss") }}</template>
            </el-table-column>
            <el-table-column prop="dmname" label="挖掘任务名称">
            </el-table-column>
