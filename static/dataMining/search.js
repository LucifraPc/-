          ...
          var result = res.data
				for(var i=0;i<result.length;i++){
					...
					if(result[i].ct==2){
						 ...
						 if(result[i].dc2.type==2){addcont+="<option value='2' selected='select'>等于</option>";}else{addcont+="<option value='2' >等于</option>";}
						 if(result[i].dc2.type==1){addcont+="<option value='1' selected='select'>小于</option>";}else{addcont+="<option value='1' >小于</option>";}
						 if(result[i].dc2.type==3){addcont+="<option value='3' selected='select'>大于</option>";}else{addcont+="<option value='3' >大于</option>";}
						 if(result[i].dc2.type==4){addcont+="<option value='4' selected='select'>范围</option>";}else{addcont+="<option value='4' >范围</option>";}
							addcont+="</select>"
					...
					if(result[i].ct==4){
						...
						for(var j=0;j<pid.length;j++){
							if(pid[j]==3){a=1;addcont+="&nbsp;&nbsp;<input type='checkbox' name='pdt_checkbox"+k+"' value='3' checked='chedked'>土建";}
						}
						if(a==0){addcont+="&nbsp;&nbsp;<input type='checkbox' name='pdt_checkbox"+k+"' value='3'>土建";}
						for(var j=0;j<pid.length;j++){
							if(pid[j]==2){b=1;addcont+="&nbsp;&nbsp;<input type='checkbox' name='pdt_checkbox"+k+"' value='2' checked='chedked'>钢筋";}
						}
						if(b==0){addcont+="&nbsp;&nbsp;<input type='checkbox' name='pdt_checkbox"+k+"' value='2'>钢筋";}
						for(var j=0;j<pid.length;j++){
							if(pid[j]==5){c=1;addcont+="&nbsp;&nbsp;<input type='checkbox' name='pdt_checkbox"+k+"' value='5' checked='chedked'>安装";}
						}
						if(c==0){addcont+="&nbsp;&nbsp;<input type='checkbox' name='pdt_checkbox"+k+"' value='5'>安装";}
					...
					if(result[i].ct==5){
						...
						if(result[i].dc5.bt == 1){
							...
							}else{addcont += "<option value='1' >已拥有</option>";}
						if(result[i].dc5.bt == 2){
							addcont += "<option value='2' selected='selected'>未购买过</option>";
							}else{addcont += "<option value='2' >未购买过</option>";}
						if(result[i].dc5.bt == 3){
							addcont += "<option value='3' selected='selected'>N天内到期</option>";
							}else{addcont += "<option value='3' >N天内到期</option>";}
						if(result[i].dc5.bt == 4){
							addcont += "<option value='4' selected='selected'>过期N天内未续费</option>";
							}else{addcont += "<option value='4' >过期N天内未续费</option>";}
                    	addcont += "</select><div style='float:left;width:650px;'>";
						
                    	var a="",b="",c="",d="",e="",f="";
                    	var proInfo = result[i].dc5.param;
                    	for(var h=0;h<proInfo.length;h++){
                    		if(proInfo[h].pt==2 && proInfo[h].pid==3){a={name:"土建云功能",day:proInfo[h].day};}
                    		if(proInfo[h].pt==2 && proInfo[h].pid==2){b={name:"钢筋云功能",day:proInfo[h].day};}
                    		if(proInfo[h].pt==2 && proInfo[h].pid==5){c={name:"安装云功能",day:proInfo[h].day};}
                    		if(proInfo[h].pt==12 && proInfo[h].pid==3){d={name:"土建BIM",day:proInfo[h].day};}
                    		if(proInfo[h].pt==12 && proInfo[h].pid==2){e={name:"钢筋BIM",day:proInfo[h].day};}
                    		if(proInfo[h].pt==12 && proInfo[h].pid==5){f={name:"安装BIM",day:proInfo[h].day};}
                    	}
                    	var proInfos = [a,b,c,d,e,f];
                    	var isAll = true;
						for(var g=0;g<proInfos.length;g++){
							if(proInfos[0].day != proInfos[g].day){isAll=false;break;}
						}
						if(isAll && proInfos.length==6 && proInfos[0].day){proInfos = "";}
                    	...
                    	for(var j=0;j<buytype.length;j++){
				             if(j==0){
				            	...
				            	else{addcont+="<input type='checkbox' name='btp_checkbox"+k+"' onclick='checkAll(this.name)' value='"+j+"'>"+buytype[j]+"&nbsp;&nbsp;";}
				             }else{
				            	 var t=0;
					...
					if(result[i].ct==8){
						...
						var addcont="时间范围：<input  value='"+result[i].dc8.st+"' id='sd"+k+"' class='Wdate startDate'  type='text' size='12' onClick=WdatePicker({errDealMode:1,readOnly:true,isShowClear:false,maxDate:'#F{$dp.$D(ed"+k+")||\\'%y-%M-%d\\'}'})> ~ "
                    			+"<input   type='text' size='12' value='"+result[i].dc8.et+"' id='ed"+k+"' class='Wdate endDate' onClick=WdatePicker({isShowClear:false,readOnly:true,minDate:'#F{$dp.$D(sd"+k+")}',maxDate:'%y-%M-%d'})>"
				                    +"&nbsp;&nbsp;账号登录次数：平均"
									+"&nbsp;<select class='lgType'>";  
						if(result[i].dc8.type==2){addcont+="<option value='2' selected='select'>=</option>";}else{addcont+="<option value='2' >=</option>";}
						if(result[i].dc8.type==1){addcont+="<option value='1' selected='select'><</option>";}else{addcont+="<option value='1' ><</option>";}
						if(result[i].dc8.type==3){addcont+="<option value='3' selected='select'>></option>";}else{addcont+="<option value='3' >></option>";}
					...
					if(result[i].ct==9){
						...
						if(result[i].dc9.type==2){addcont+="<option value='2' selected='select'>=</option>";}else{addcont+="<option value='2' >=</option>";}
						if(result[i].dc9.type==1){addcont+="<option value='1' selected='select'><</option>";}else{addcont+="<option value='1' ><</option>";}
						if(result[i].dc9.type==3){addcont+="<option value='3' selected='select'>></option>";}else{addcont+="<option value='3' >></option>";}
...
var addLeft = function addLeft(ct,k,condition,addcont){
       ...
       for(var cn in condition){
    	   		if( ct == cn){newTr+="<option value='"+cn+"' selected='selected' >"+condition[cn]+"</option>";}
    	   		else{newTr+="<option value='"+cn+"' >"+condition[cn]+"</option>";}  	   
       }
                newTr+= "</select>"
                + "</td>"
                    			...
                    			+"<input   type='text' size='12' value='"+funUse.et+"' id='ed"+k+"' class='Wdate endDate' name='fet' onChange='fetChange(this)' onClick=WdatePicker({isShowClear:false,readOnly:true,minDate:'#F{$dp.$D(sd"+k+")}',maxDate:'%y-%M-%d'})> <br/>";
						         addcont+="<select class='functionType' onchange='funChange(this,"+k+")' style='float:left' >";
						            ...
						            if(funCheck[0].fn=="不限功能"){
								            ...
								            if(funCheck[0].type==3){addcont+= "<option value='3' selected='select'>></option>"}else{addcont+= "<option value='3'>></option>"}
									        if(funCheck[0].type==1){addcont+= "<option value='1' selected='select'><</option>"}else{addcont+= "<option value='1'><</option>"}
									        if(funCheck[0].type==2){addcont+= "<option value='2' selected='select'>=</option>"}else{addcont+= "<option value='2'>=</option>"}
								                    addcont+=                    "</select>"
						         ...
						         for(var i=0;i<results.length;i++){
						        	 		...
						        	 		for(var b=0;b<funCheck.length;b++){
						        	 			...
						        	 			if(results[i].functionName==funCheck[b].fn && results[i].productId == funCheck[b].pid){
									                ...
									                if(funCheck[b].type==3){addconts+= "<option value='3' selected='select'>></option>"}else{addconts+= "<option value='3'>></option>"}
									                if(funCheck[b].type==1){addconts+= "<option value='1' selected='select'><</option>"}else{addconts+= "<option value='1'><</option>"}
									                if(funCheck[b].type==2){addconts+= "<option value='2' selected='select'>=</option>"}else{addconts+= "<option value='2'>=</option>"}
									                addconts+=                "</select>"
